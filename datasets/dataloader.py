...
from .dataset import ImageSet, UnsupImageSet, SemiImageSet
...
class SemiDataloader(data.DataLoader):
    def __init__(self, config, root_dir, batch_size):
        self.root_dir = root_dir
        self.dataset = SemiImageSet(
            root_dir=root_dir, 
            transforms=get_augmentation(config, _type='train'),
            unsup_transforms=get_augmentation(config, _type='val')
        )
        ...
        super(SemiDataloader, self).__init__(
            self.dataset,
            batch_size=batch_size,
            num_workers=2,
            pin_memory=True,
